...
{	
	...
	*/
	public function tree(){
		...
		foreach($users as $user){			
            $html .= $this->get_team1($user['userid'],$user['realname'],$user['phone']);
        }
		$html .= '</ul>';
	...
	public function get_team1($userid,$realname,$phone='',$new=true)
    {
        static $data = '';
        if($new){
            $data = '';
		...
		*/
        if($users[0]) {
			if($new){
				$data .= '<li><span><i class="icon-minus-sign"></i> '.$userid.'-'.$phone.'-'.$realname.'</span><ul>';
			}
            foreach ($users as $user) {
				
                if ($user) {					
                    //有下级
                    $data .= '<li><span><i class="icon-minus-sign"></i> '.$user['userid'].'-'.$user['phone'].'-'.$user['realname'].'</span><ul>';
                    $this->get_team1($user['userid'],$user['realname'],$user['phone'],false);
                    $data .= '</ul></li>';
                }
            ...
            //没有下级
            if($new){
				$data .= '<li><span><i class="icon-leaf"></i> '.$userid.'-'.$phone.'-'.$realname.'</span></li>';
			}
